# E2E milestone · play loop and continent entry
...
```bash
mkdir -p /tmp/unwritten-play-loop-replay
AETHER_TEST_USER_DATA=/tmp/unwritten-play-loop-replay bun run dev --remoteDebuggingPort 9441
CDP_PORT=9441 bun scripts/cdp-drive.ts "$(cat docs/e2e/milestone-play-loop/run.json)"
```

The final run used a newly empty `/tmp/unwritten-play-loop-replay`. It took about 32 seconds
