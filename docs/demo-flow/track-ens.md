...
| 要求 | 我們的對應 | 狀態 |
| --- | --- | --- |
| 建在 ENSv2（Sepolia）上 | 「用 ENS 名稱開啟」和用名稱走進大陸經 ENSv2 Universal Resolver 讀取（地址用 viem 內建的 Sepolia 設定，不寫死；`src/renderer/identity/ensNames.ts`）；市場與卡帶畫面直接讀 registry 自己的 Permissioned Resolver；ENSv2 的合約地址在 `src/main/chain/ensCalls.ts` 的 `ENSV2_SEPOLIA`。合約部署在 Sepolia 的 `unmapped.eth` 底下 | 已部署（v2：[lineage-names](../e2e/milestone-lineage-names/result.md)；2026-09-26 在區塊 11,785,145 全部重讀過：[chain-audit](../e2e/milestone-chain-audit/result.md)） |
...
| 階層式 registry：萬用字元解析子名稱，或部署自己的子名稱 registry | 名字先有：卡帶 `<cartridge>.unmapped.eth`、改編 `<remix>.<parent>.unmapped.eth`、存檔 `<save>.<cartridge>.unmapped.eth`（由玩家的 passkey 帳戶持有）；每個世界有自己的子名稱 registry | v2 在 Sepolia 上真實登記了 10 個名字：根層的 `aether-land`、`misty-harbor` 與玩家名稱目錄 `players`，2 個改編（`moss-hollow`、`lantern-quay`），4 個存檔，1 個玩家名稱（2026-09-26 區塊 11,785,145 讀取）；限制條件在模擬中通過 |
...
| live demo 連結、GitHub 開源 | repo 公開：github.com/p2p-solidarity/unmapped；live 連結：https://unmapped-auction.gimmychang.workers.dev/#aether-land（唯讀、直接讀 Sepolia） | 完成 |
...
| 時間 | 做什麼 | 說什麼 |
| --- | --- | --- |
| 0:00–0:40 | 創造世界：輸入「霧之港」和一句話 → 建立 → 「你的世界已經發布」面板：自動標籤是 `xn-9iq609e681a-…`，改成一個還沒被用過的英文標籤（下稱 `<你的標籤>`）→ 用 passkey 登記名稱（先排練：建立要約 1 分鐘，可先建好停在這個面板）。**`misty-harbor` 已被 2026-09-26 的 E2E 登記**（持有人 `0xfBaB…8537`），面板會說「這個名稱已屬於另一個世界，請換一個標籤。」 | 「AI 剛寫完一個世界。它的內容算成一個 sha256 雜湊——那是它的身分，但人記不住。所以建完的第一件事是給它一個 ENSv2 名字。中文名字沒辦法直接當標籤，玩家自己挑一個。」 |
...
| 1:10–1:50 | 進入世界：HUD 玩家卡片上是 `<你的標籤>.unmapped.eth`；走到第一章入口，跟三個人說話、找到東西 → 過關卡片「要把這趟旅程記錄到 ENS，名稱為 … 嗎？」→ 用 passkey 記錄 → HUD 變成存檔的名字 ✓ | 「名字不是選單裡的設定，是遊戲的一部分：每過一章，這趟旅程的名字就可以往前移到新的進度點。鏈上只有雜湊、版本、一行進度和門牌。」 |
| 1:50–2:30 | 朋友的 app：世界 → 大陸 → 在「夥伴的門牌或 ENS 名稱」輸入 `<存檔名>.<你的標籤>.unmapped.eth` → 穿過這扇門 → 1–2 秒後出現在同一片大陸 | 「朋友不用記六碼門牌：存檔的名字的 description 就寫著門牌，app 用 Universal Resolver 讀出來再加入。」 |
| 2:30–3:10 | 卡帶 → 無界之地 → 改編 → 發布 → 名字自動掛在 `aether-land.unmapped.eth` 底下（標籤不能是已被用掉的 `moss-hollow`、`lantern-quay`）→ 登記 → 上架（底價用父世界的代幣） | 「改編就是父名字底下的子名字，拍賣用父世界的代幣計價；交易的權利金沿著家族樹往上付給每個名字的持有人。」 |
| 3:10–3:40 | 世界 → 市場：已上架的世界（2026-09-26 有 3 個，再加上台上新上架的）；切到公開拍賣頁的家族樹 | 「registry 只保留兩個角色，每個名字都是解放的 ERC-1155，可以安全轉手——轉手之後權利金跟著新持有人。」（轉手只在模擬驗過，不要現場做。） |
| 3:40–4:00 | 回到遊戲 | 「對 AI 生成的內容來說，最難的是出處：誰做的、從哪一版來、玩到哪裡。ENSv2 的階層正好就是血緣，連一趟旅程都有自己的位置。」 |

玩家自己的名字（`<名字>.players.unmapped.eth`，在「世界 → 市場」用 passkey 認領，之後在大陸上別人看到的就是這個名字）：操作者已登記 `players.unmapped.eth` 目錄（`bun run lineage:demo players`，tx `0x8f88cbdd…87b7`，區塊 11,784,427），app 內認領 `kidney.players.unmapped.eth` 已實機驗證（[milestone-ens-players](../e2e/milestone-ens-players/result.md)）。

操作細節（按鈕、簽名、要等多久）以 `docs/demo/lineage-market.md` 為準。

### 鏈上現況（v2，Sepolia 真實交易：[milestone-lineage-names](../e2e/milestone-lineage-names/result.md)、[milestone-lineage-relay](../e2e/milestone-lineage-relay/result.md)；2026-09-26 區塊 11,785,145 重讀：[chain-audit](../e2e/milestone-chain-audit/result.md)）
...
| 權利金付給名字持有人 | v2 上真實：5.338 AETHERLAND 付給名字持有人 `0x8eEC…51C3`（tx `0xa2efd93f…86bd`）；v1 上也做過 |
...
| `misty-harbor` 與 `lantern-quay` 的 100 區塊拍賣 | 都在 0 筆出價下結束（募得 0，門檻 10），沒有畢業、沒有池子；它們的卡帶只存在當時 E2E 的資料夾，別台機器「用 ENS 名稱開啟」會要求匯入一個不存在的 `.cartridge` |
| 玩家名稱目錄 `players.unmapped.eth` 與 `kidney.players.unmapped.eth` | 真實（[milestone-ens-players](../e2e/milestone-ens-players/result.md)） |
...
- 線上代付站已是這一版的規則（2026-09-26 06:23 UTC 重新部署，version `f539261a`），會代付 app 內的上架；不過還沒有真的經由線上代付站上架過（app 內上架只在本機代付站跑過，線上只做過偽造請求的探測）。
