...
- 市場：根世界 `aether-land.unmapped.eth` 已經拍賣完、池子已開（[milestone-lineage-demo](../e2e/milestone-lineage-demo/result.md)）；中繼器 Sepolia ETH 維持 0.02 以上；passkey 帳戶先領好測試 USDC。
...
- **畫面**：Worlds → Market（世界 → 市場）→ 點 `aether-land.unmapped.eth`：名字、持有人（Name held by …）、拍賣結果。切到公開拍賣頁 https://unmapped-auction.gimmychang.workers.dev 的家族樹。
...
- **投影片 3（4 點）**：
  ...
  - A remix is a subname with its own registry
  - Names stay emancipated — safely transferable tokens
- **講稿**：
  > Every published world is an immutable, hashed cartridge on your disk. On ENSv2 it gets a name — aether-land.unmapped.eth — whose records hold only its id, version and hash. A remix becomes a subname: launching it deploys the remix's own registry under its parent, so the family tree is the ENS hierarchy. We keep only two roles on those registries, so every world name stays emancipated and safely transferable.
...
- **講稿**：
  > Each world's token is launched with Uniswap's Continuous Clearing Auction, priced in its parent's token. Our first world took five passkey bids and cleared 84% above its floor, then graduated into a v4 pool. I buy with a passkey — no wallet, no ETH; our relayer only pays gas and can't change what I signed. Our hook took exactly one percent, and it pays whoever holds the world's ENS name.
- 實測（Sepolia 真實交易，[milestone-lineage-demo](../e2e/milestone-lineage-demo/result.md)）：5 筆出價、清算 0.018395 USDC（+84%）、募得 9,070 USDC；10 USDC 買到 535.39，hook 抽 5.408（剛好 1%）；分潤付給名字持有人。
