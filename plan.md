...
不同卡帶也能合併：只分享原點場景、顯影層與家門，規則／故事／委託／怪物不過線。實作：`src/shared/continent.ts`、
`src/renderer/net/continent*.ts`。y-webrtc glare 以 patch 修正（只有 peerId 較小的一方發 offer）。
...
- 戰鬥邏輯抽成 `combatLoop.ts`，3D 與開放地共用。開放地：面向方向開火（點擊，以及平地上沒用途的跳／手電筒鍵），
  原點怪 + 周圍 3×3 區塊的野怪（離家越遠越強），HUD 顯示 HP／彈藥／擊殺；兩種畫法都畫怪的等級、血條與彈道。
  實機：7 發 → 擊倒 1 隻（1 xp），切 16-bit 再擊倒 1 隻。即時制怪不反擊（與 3D 一致）；回合制時怪會在自己的回合開火（沿用 3D 邏輯，沒在開放地實測）。
...
- 實機（燈塔守夜人 3.0.0）：「北邊…舊堤防步道，有史萊姆」→ 海霧舊堤防（北 1 區塊）：開寶箱得 2 材料、擊倒 1 隻、跑到終點回地面 ✓；
  「東邊…地下倉庫，被骷髏佔了」→ 舊燈芯倉庫（東 1 區塊）：第一人稱走格子、手電筒、從回程出口回到入口，地面戰鬥恢復。
...
- 地形在碰撞上是平的；HD-2D 只在畫面上把岩地畫成台地、液體畫成盆地（走上台地時角色平滑抬高）。真正的高低差規則仍未做。
- 湖面不可行走；每個區塊中央的淺灘保證出生點與所有閘門相連（`isFord`）。要不要船仍未決定。
