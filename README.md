...
- **Chat.** On friends' joined worlds and in a world shared through a world service, **Press Enter to talk**: <kbd>Enter</kbd> opens the line,
  ...
  only: never saved and never in a history. In a shared world the service relays each line to
  friends there and keeps nothing; its owners and members talk, visitors only read.
...
| Chat in a shared world, through its world service (`unmapped-service/2`): owners and members talk, the service relays to friends that can hear and keeps nothing; an older app or service never gets a chat frame | ✅ verified: invite link made in 0.5 s with no server shown, joined in 0.7 s, a line A → B in 0.92 s, typing never walks, the lines are in no file of the service or either app | [simplify-world-chat](docs/e2e/milestone-simplify-world-chat/result.md) |
