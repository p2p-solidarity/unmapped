...
export async function pollHealth(
  ...
  while (Date.now() < deadline) {
    if (!isAlive()) {
      return fail({
        code: "sidecar-exited",
        message: "llama-server exited before becoming ready.",
      });
    }
    try {
  ...
  return fail({
    ...
    message: `llama-server did not report healthy on :${port} within ${deadlineMs / 1000}s.`,
...
export function createSidecar(ctx: MainContext): Sidecar {
  ...
  async function start(config: SidecarConfig | null): Promise<Result<SidecarStatus>> {
    ...
    if (!health.ok) {
      ...
      setStatus({ state: "error", pid: proc.pid ?? null, port: config.port, message });
      await stop();
