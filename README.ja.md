...
には `--browser-origin <ページのオリジン>` が必要です。組み込みのカートリッジから始めたワールドも、持ち主が
共有したとき（または以前に共有したワールドを次に開いたとき）にパックを公開するので、スマートフォンで描けます。
...
| スマートフォンサイズのブラウザー実証：招待で参加、大地、タッチで歩く、オフラインと再読み込み後のメモ、デスクトップのプレイヤーを見る | ✅ 検証済み（ヘッドレスブラウザー 375 × 812、開発版のページ）。組み込みのカートリッジから始めたワールドも含む。実機では未実行 | [p4-mobile-proof](docs/e2e/milestone-rev6-p4-mobile-proof/result.md) |
