...
呼ばずに歩けます。1 台の端末だけにあるワールドも、P2P の WebRTC で友だちのワールドとつながれます。参加
コードか ENS 名を友だちに伝えるだけです。
...
- **かんたんに始められる。** タイトルは「つづきから」「ワールド」「世界をつくる」「設定」の 4 つだけです。
  **ワールド → マイワールド** はひとつの一覧で、**新しい冒険をはじめる**、遊んだワールドごとの**再開**、
  入っているけれどまだ遊んでいないワールドの**始める**が並び、ほかはすべて各行の**もっと**の中です。プレイ中の
  プレイヤーカードには、ワールドの名前（ENS 名があればそれを先に）と、ひとことの**目標**が出ます。初めて遊ぶ
  ときは**遊び方**のカードが開き、ドックの**遊び方**ボタンでまた開けます。2026-09-26 に実装し、アプリでは
  まだ実行していません（[状態表](#現在の状態)）。
...
- **共有ワールド。** ワールドは署名付きで追記しかできない歴史で、オフラインを優先します。ワールドサービスで
  ...
  だれも訪れない場所は霧に還って伝説になり、住人は友だちが実際にしたことを噂として語ります。友だちは
  贈り物や道しるべを残し、互いの歩く姿やエモートを見られます。サービスは署名済みの記録を並べ、検査し、
  中継するだけで、モデルを呼ぶことはありません。
- **友だちを招待、またはワールドに参加。** ひとつの欄に、友だちの ENS 名か 6 文字の参加コードを入れるだけ
  です。1 台の端末だけにあるワールドは P2P で友だちに開かれ、ワールド同士が y-webrtc でひとつの大地に
  つながります。誰かのワールドを預かるゲームサーバーは存在しません。友だちは名前、向き、歩く姿つきで表示され、
  F12 → 友だち にはそれぞれの居場所と距離が並び、<kbd>Enter</kbd> でチャットを開けます。ピアから届くデータは
  すべて、書き込まれる前に検証されます。
- **モデルの呼び出しは 1 回に 1 経路。** ローカルモデル、自分のキー、または UNMAPPED 生成ゲートウェイの
  アカウントの無料枠のどれかです。「設定 → モデル」に次の呼び出しの行き先が表示され、失敗しても別の経路に
  切り替わることはありません。
- **画像はライセンスを覚えている。** 「設定 → 詳細設定 → 画像」で描き手を選びます。どの画像にもライセンスが
  記録され、商用モードでは販売できないプロバイダーや新しい画像を拒みます。
- **サーバーがなくなってもワールドは残る。** `.world` ファイルには、ワールドの歴史のすべてと必要なパックが
  入っています。だれでもオフラインで検証し、別の端末やワールドサービスで立ち上げ直せます。
...
- **データはあなたのもの。** 公開したコンテンツは、sha256 ハッシュ付きで変更できないカートリッジになります。
  ...
  `.world` で共有ワールドを別のマシンへ移せます。暗号化したシードの書き出し
  （`.seed.enc`、F12 → ワールド）はランダムな AES-GCM データキーで封じられ、そのキーはパスキーの PRF か OS の
  キーチェーンで保護されます。
...
- **正直な数字。** モデルの呼び出しはすべてローカルの使用量台帳に記録されます。記録するのはトークン数、
  キャッシュされたトークン数、ミリ秒で、プロンプトやキーは記録しません。F12 → 推論 にはワールドごとの累計が
  出ます。ダミーのデータは一切表示しません。足りないものがあれば、画面がそのことと直し方を伝えます。
- **オンチェーンの来歴はオプション。** ワールド、リミックス、セーブ、プレイヤーは Sepolia 上で ENSv2 の名前を
  持てます。名前はゲームの中にもあります（プレイヤーカード、章のクリア、名前で友だちのワールドに参加）。ひとつの
  パスキー（設定 → あなたのパスキー）が名前を持ち、マーケットの操作も確認します。共有ワールドのサービスは、拍ごとの
  指紋をチェーンに記録することもできます（Sepolia にデプロイ済み）。コンテンツ自体がチェーンに載ることはなく、
  チェーンを設定しなくてもすべての画面が動きます。
...
<table>
  ...
  <tr>
    <td><sub><b>友だちのワールド。</b>友だちのワールドが WebRTC でつながり、名前・向き・歩く姿つきで表示されます。</sub></td>
  ...
  <tr>
    ...
    <td><sub><b>扉。</b>だれが入れるか、招待リンク、共同の持ち主は、いまは扉の<b>詳細</b>の中にあります（扉を簡単にする前の画面です）。</sub></td>
  </tr>
</table>
...
起動したら **タイトル → 設定 → モデル** を開き、文章を書かせるモデルを選びます。それから
**ワールド → マイワールド → 新しい冒険をはじめる** で遊ぶか、**世界をつくる** で自分のワールドをつくります。
...
| 無料枠（UNMAPPED 生成ゲートウェイ） | `UNMAPPED_GATEWAY_URL`（開発版にはなし） | 設定 → 詳細設定 → アカウント、または `.env` の `UNMAPPED_GATEWAY_KEY` | 経路・計量・キャンセル・画像 ✅（テスト用の上流のみで検証、[p4-quota](docs/e2e/milestone-rev6-p4-quota/result.md)） |
...
| 場面 | キー |
| --- | --- |
| 大地 | <kbd>W</kbd><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd> またはクリックで移動 · <kbd>Shift</kbd> ダッシュ · <kbd>E</kbd> 話す／開ける · <kbd>N</kbd> メモ · <kbd>V</kbd> 表示切替 · <kbd>F12</kbd> 友だち・その他（コンソール。開くと最初は「友だち」） |
...
| 友だちと（つながったワールド） | <kbd>Enter</kbd> 話す · <kbd>Enter</kbd> 送信 · <kbd>Esc</kbd> 閉じる |
| 共有ワールド（オンライン） | <kbd>T</kbd> エモート（パッドは <kbd>LB</kbd>）· <kbd>1</kbd>〜<kbd>6</kbd> で直接選ぶ |
| スマートフォン（ブラウザー実証） | スティック：歩く · Y：このワールドとメモ |

HUD には、いまいる場所で必要なキーだけが表示されます。<kbd>N</kbd>・<kbd>V</kbd>・<kbd>F12</kbd> はドックの
ボタンにあります。初めて遊ぶときは**遊び方**のカードが基本を示し、ドックの**遊び方**ボタンでまた開けます。
...
アプリでは、**ワールド → マイワールド** の各ワールドの**もっと**で `.world` と `.cartridge` の書き出しと
バックアップができ、一覧そのものの**もっと**でバックアップから復元したり `.cartridge` を読み込んだりできます。
`.world` は **ワールド → ワールドに参加 → もっと：.world ファイルから参加** で取り込みます。
...
友だちと遊ぶのに必要なのは、**友だちを招待**と**ワールドに参加**の 2 つのボタンと、相手に伝えるひとつの
ことばだけです。ワールドに ENS 名があれば**ENS 名**を、なければ 6 文字の**参加コード**を伝えます。これらの
画面は 2026-09-26 に作り直したもので、アプリではまだ実行していません（[状態表](#現在の状態)を参照）。その下の
しくみは実行済みです。

- **友だちを招待。** 家に帰って扉の前で <kbd>E</kbd>（**扉を開ける**）を押します。いちばん上が
  **友だちを招待**です。押したら、表示される名前か参加コードを友だちに伝えてください（**コピー**でコピー
  できます）。開いているあいだは**開いています · 友だち N 人**と表示され、**閉じる**で閉じます。同じ欄は
  <kbd>F12</kbd> → **友だち** の最初にもあり（**友だちを呼ぶ**）、**ワールド → マイワールド** で遊んだ
  ワールドの**もっと**にも**友だちを招待**があります（先にそのワールドを開きます）。
- **ワールドに参加。** 友だちはその名前か参加コードを**ワールドに参加**に入力します。場所は自分の扉の前、
  F12 → 友だち、または遊ぶ前の **ワールド → ワールドに参加** です。最後のものは招待リンク
  （`unmapped://join?…`）や引っ越しリンクも受け付けます。ENS 名は最初に照会され、セーブの名前ならその参加
  コードへ、ワールドの名前なら**このワールドで遊ぶ**へ進みます。参加コードで参加するときは、自分のワールドを
  ひとつ連れていきます（**連れていく：…** · **変更**。まだワールドがなければ、先に組み込みのワールドで新しい
  冒険が始まります）。あなたのワールドが友だちのワールドの隣に行き、大地も進み具合もそのままです。
- **オンラインの友だち。** F12 を開くと最初は**友だち**です。招待、参加、そして**オンラインの友だち**：この
  大地に描かれているほかのプレイヤー全員の位置（`x, z`）、だれの土地にいるか、あなたから何歩か、が 1 秒に
  4 回更新されます。
- **チャット。** 友だちとつながったワールドと、ワールドサービスで共有したワールドでは **Enter で話す**：<kbd>Enter</kbd> で入力欄が開き、
  <kbd>Enter</kbd> で送信、<kbd>Esc</kbd> で閉じます。1 行は最大 200 文字で、検証済みの友だちにだけ届きます。
  友だちひとりにつき 5 秒で最大 5 行です。チャットはメモリの中だけにあり、保存も、歴史への記録もされません。
  共有ワールドではサービスが各行をその場の友だちに中継し、何も残しません。話せるのは持ち主とメンバーで、訪問者は読むだけです。
- **扉のそのほか。** ワールドに参加の下には、**友だちのワールド**（**その扉へ行く**）、4 つの
  **クイック移動**（行ったことのある場所か、友だちの参加コード）、持ち帰った記念品、訪問者があなたの大地に
  残したメモ（**残す**）が並びます。たたまれた**詳細**には、ワールドの置き場所、**招待リンク**、
  **だれが入れるか**、**一緒に遊ぶ人**、**公開チェーンへの記録**、**この世界の記録**があります。サービスの
  アドレスは、設定のほかではどこにも表示されません。

### つながったワールドのしくみ（大陸）

1 台の端末だけにあるワールドは、P2P で友だちとつながります。参加コード（コードの中と以前のビルドでは扉番号、
`plateOf`）は変わらず、そのワールドが開く**大陸**のコードでもあります。大陸とは、y-webrtc でひとつの大地に
つながったワールドを指す技術上の名前です。ワールドサービスで共有したワールドは大陸に参加できず
（`continent-world-attached`）、その**友だちを招待**は招待リンクへ案内します。ワールドはそれぞれ自分の原点、
シード、セーブを持ったままです。参加するとアンカーが割り当てられ、各地はいちばん近いアンカーの領地になります。
セーブの ENS 名にはその参加コードが記録されていて、名前から参加コードにたどり着けるのはそのためです。

- **やりとりされるもの：** 各ワールドの概要、観測済みのチャンク、メモ、リアルタイムの位置（awareness のみで、
  保存はしません）、チャット。訪問者があなたの大地に残したメモは扉で待ち、あなたが**残す**を選ぶと、あなたが
  署名したメモとしてワールドの歴史に加わります。
...
- **信頼：** ピアの hello が大陸コード、プロトコル、物理バージョンと一致するまで、何も交換しません。その後も
  ...
  残せますが、既存のチャンクやメモを上書きすることは決してできません。チャットは両端で整えられて長さを制限
  され、話した人の名前はメッセージからではなく、そのピア自身のプレゼンスから取ります。
- **シグナリング：** 既定では公開の y-webrtc サーバーを使います。自分で立てることもできます
  （`PORT=4444 node node_modules/y-webrtc/bin/server.js`）。**設定 → 詳細設定 → シグナリングサーバー** に
  追加すれば、保存する前にそこで接続をテストできます。
...
だけにあり、オフラインで遊べます。ワールドサービスで共有するには、**設定 → 詳細設定 → 共有ワールド** に
ワールドサービスを並べ、扉の**詳細**を開いて**招待リンクをつくる**を選びます。この端末だけにあるワールドは、
まずそのリストの最初のサービスで共有されます。扉にそのサービスの名前は出ず、リストが空なら招待リンクは
出てきません。サービスは記録を並べ、アプリと同じ方法で 1 件ずつ検査し、中継し、あなたがいないあいだも
預かります。モデルを呼ぶことはなく、モデルのキーも持ちません。`bun run service` で自分で立てられます
（[開発](#開発)を参照）。

- **招待リンク。** **招待リンク**の欄で**招待リンクをつくる**を押すと、使える人数（1–20）と日数（1–30）の
  決まったリンク（`unmapped://join?…`）ができます。表示されるのはこの一度だけです。友だちは
  **ワールド → ワールドに参加** に貼り付け、**ワールドを見る**、**参加して遊ぶ** の順に選びます。使い切った
  招待、期限切れの招待、取り消した招待は拒否されます。
- **だれが入れるか。** **自分だけ**、**友だち**（既定：あなたと招いた友だち）、**だれでも**（ワールドの id を
  知っていればだれでも訪れて、メモ・道しるべ・贈り物を残せます。場所を書けるのは招いた友だちだけ）。
  **一緒に遊ぶ人**の欄では、持ち主が友だちを**外す**ことができ、外された人はそのワールドを読むことも書くことも
  できなくなります。書いたものは残ります。**共同の持ち主にする**と、別の端末に持ち主の権限がすべて渡り、
  端末を 1 台なくしてもワールドは続きます。
...
- **拍：霧、伝説、季節。** 拍は 6 時間ごとに刻まれ、季節は 7 日ごとに変わります。町のまわりより外にあって、
  ...
  拍を刻みます。
- **噂。** 拍は、友だちが章をクリアしたといった実際の出来事を選び、住人がそれを語り継ぎます（「噂では…」）。
  噂が触れてよいのは引用した出来事だけで、ほかのことに触れた噂はアプリもサービスも拒否します。拍ごとの噂は、
  端末が自分のモデルで書きます（呼び出しは多くて 1 回）。書くかどうかは
  **設定 → 詳細設定 → 共有ワールド → バックグラウンドで噂を書く** で決まり、既定では自分が持ち主のワールドで
  だけ書き、無料枠は決して使いません。
- **贈り物、プレゼンス、エモート。** 最初に来た人への贈り物を置けます。ふたりが同時に受け取ろうとすると、
  受け取れるのはひとりだけで、もうひとりには「先に誰かが受け取りました。」と表示されます。オンラインの友だちは
...
- **古いセーブもそのまま。** このビルドが古いセーブを初めて開くと、セーブの大地、伝承、メモ、場所、章、行いを
  ...
  その書いた分を取り込みます。
- **サーバーがなくなってもワールドは残る。** **ワールド → マイワールド** の各ワールドの**もっと**に
  **.world を書き出す** があり、**ワールド → ワールドに参加 → もっと：.world ファイルから参加** でファイルを
  オフラインで確かめてから**取り込む**ことができます。ワールドのサービスがなくなったら、そのファイルを別の
  ワールドサービスに取り込みます（`bun run service -- import`）。持ち主がワールドをそのサービスへ移すための
  フォームは、いまは表示されません（機能はコードに残っています）。引っ越しリンクを受け取った友だちは、それを
  **ワールドに参加**に貼り付けて**ワールドを追いかける**を選びます。新しい物理バージョンでつくられたワールドや
  ファイルは拒否されます。

## アカウント、無料枠、画像

ローカルモデルや自分のキーで遊ぶなら、どれも必要ありません。これらの設定は **設定 → 詳細設定** の中に
あります。ゲートウェイが設定されていなければ、そこの**アカウント**と**プラン**がそのことを伝え
（`gateway-not-configured`）、「設定 → モデル」に無料枠は出てきません。このアカウントが管理するのはこの
デバイスのゲートウェイでの無料枠だけで、パスキーとは別のものです。
...
- **無料枠。** 呼び出しはまずクレジットを確保し、使ったトークンに応じて精算します。キャンセルした呼び出しは確保を
  ...
  選択肢を示します。**設定 → 詳細設定 → プラン** には、ゲートウェイの課金プロバイダー（Stripe）のプランが並びます。課金
  プロバイダーのないゲートウェイは何も売りません（`billing-not-configured`）。
- **画像。** **設定 → 詳細設定 → 画像** で描き手を選びます。OpenAI、指定したサーバー上の Qwen-Image-2512 か
...
- **ワールド・セーブ・プレイヤーの ENSv2 名（Sepolia）。** すべて `unmapped.eth` の下の一本のツリーに付きます。
  ...
  保有します。それが **設定 → あなたのパスキー** のひとつのパスキーで、ボタンひとつ（**パスキーを設定**）で
  つながり、その下にあなたのプレイヤー名が出ます。名前はメニューの中だけでなくゲームの中にあります。
  **作って遊ぶ** の直後にワールドへ名前を付けられ（ラベルは自分で選ぶので「霧之港」は
  `misty-harbor.unmapped.eth` にできます）、そのままマーケットに出せます。プレイヤーカードのいちばん上には
  ワールドの名前（ENS 名があればそれを先に）が出て、章をクリアするとパスキーの署名一回で旅を記録するか、名前を
  新しい進行状況へ進めるよう提案されます。友だちは**ワールドに参加**にあなたのセーブの名前を入力するだけで、
  あなたのワールドに参加できます。**ワールド → マイワールド** の各ワールドの**もっと**ではリビジョンへの命名、
  向け直し、マーケットへの出品、旅の名前の記録ができ、**ワールドに参加** はワールドの名前から正確なリビジョン
  （**このワールドで遊ぶ**）へ、セーブの名前からその参加コードへたどり着きます。レコードは id、バージョン、
  コンテンツハッシュだけ（セーブはその sha256、固定したバージョン、進行状況の一行、参加コード）で、別の端末で
  復元したバックアップはハッシュから自分の名前を見つけます。
...
- **共有ワールドの軽いチェーン（Sepolia）。** 持ち主は、扉の**詳細**にある**公開チェーンへの記録**で
  **記録する**を選べます。するとワールドサービスが拍ごとの指紋（ワールドの id、記録の番号、ハッシュ）を
...
- **系譜マーケット（実験的・Sepolia）。** 名前の付いたカートリッジは発行できます。トークンは親ワールドの
  ...
  （**ワールド → マイワールド** の各ワールドの**もっと**、または作った直後）。リミックスは親が出品してからで、親のトークン建てです。
...
UNMAPPED はバージョン `0.2.1`、動作はする初期段階の研究版です。実際のアプリで動かして確かめた機能だけを
...
| ゲームの中の ENS: 作った直後の命名（中国語名は自分でラベルを選ぶ）、アプリからの出品、プレイヤーカードの名前、章のクリア後に旅の名前を記録・更新（扉番号つき）、リミックスボタンから作ったリミックスを親の名前の下で命名し親のトークン建てで出品、友だちがセーブの名前で大陸に参加 | ✅ Sepolia で検証済み（Touch ID は仮想認証器で代用、ガスステーションはこのビルドをローカルで実行）。章のカードの更新ボタンは押せていない（同じ更新はセーブ画面から）（メニュー名はその後変更。上記参照） | [ens-in-game](docs/e2e/milestone-ens-in-game/result.md) |
...
| 扉：非公開・仲間・公開、1 回限りの招待（使い切り・期限切れ・取り消しは拒否）、メンバーを外す（メニュー名はその後変更。上記参照） | ✅ 検証済み。修正後はメインプロセスも署名の前に外されたメンバーの書き込みを拒み、送信待ちだった記録は拒否として表示される | [p3-door](docs/e2e/milestone-rev6-p3-door/result.md) |
...
| **ワールド → マイワールド** をひとつの一覧に：先頭に新しい冒険をはじめる、セーブごとに再開、まだ遊んでいないワールドごとに始める。友だちを招待（遊んだワールドから）、ENS 名、`.world` の書き出し、バックアップ、更新、古いバージョン、下書き、リミックス、`.cartridge` の書き出し、出品は各行のもっとの中。バックアップから復元とワールドの読み込みは一覧のもっとの中 | ✅ 検証済み：一覧、新しい冒険をはじめる（2.7 秒でプレイへ）と再開。その他の中の操作は今回押していない | [simplify-one-world](docs/e2e/milestone-simplify-one-world/result.md) |
| **ワールド → ワールドに参加** のひとつの欄：ENS 名を最初に照会（セーブの名前 → その参加コード、ワールドの名前 → このワールドで遊ぶ）、それ以外は参加コード（自分のワールドをひとつ連れていく）、招待リンク、引っ越しリンク。`.world` ファイルはもっとの中 | ✅ 参加コードで検証（VPN オン、リレー経由 2.2 秒で接続）。ENS 名、招待リンク、引っ越しリンクは今回未実行 | [simplify-play-together](docs/e2e/milestone-simplify-play-together/result.md) |
| **F12 → 友だち**（コンソールの最初のタブ。家の扉にも同じ欄）：友だちを招待、ワールドに参加、オンラインの友だち（それぞれの位置、だれの土地にいるか、何歩離れているか）。ルームのパネルは削除 | ✅ 検証済み：招待、参加、歩くと更新されるオンラインの友だちの位置 | [simplify-play-together](docs/e2e/milestone-simplify-play-together/result.md) · [simplify-one-world](docs/e2e/milestone-simplify-one-world/result.md) |
...
| ワールドサービスで共有したワールドでのチャット（`unmapped-service/2`）：持ち主とメンバーが話せ、サービスは聞ける友だちにだけ中継して何も残さない。古いアプリや古いサービスはチャットのフレームを受け取らない | ✅ 検証済み：サーバーを見せずに招待リンクを 0.5 秒で作成、0.7 秒で参加、A → B の 1 行が 0.92 秒、入力中は歩かない、サービスと両アプリのどのファイルにもチャットは残らない | [simplify-world-chat](docs/e2e/milestone-simplify-world-chat/result.md) |
...
└── renderer/
    ├── app/        画面と HUD：タイトル、ワールド一覧、つくる、プレイ、大地、友だち、コンソール
    ...
    ├── net/        大陸（友だちとつながったワールド）：y-webrtc のルーム、ゲート、シグナリング、チャット。
    │               ワールドサービス。プレゼンス
